...
import { useLatestAIAnalysis } from '@/hooks/useDashboardData';

export function AIRiskScore() {
  const { data: aiData } = useLatestAIAnalysis();
  
  const riskScore = aiData?.risk_score ?? 0;
  const anomaliesDetected = aiData?.anomalies_detected ?? 0;
  const threatsBlocked = aiData?.threats_blocked ?? 0;
  const predictions = (aiData?.predictions as any[]) ?? [];
  const recommendations = (aiData?.recommendations as any[]) ?? [];
  ...
  return (
    <div className="panel gradient-threat">
      ...
      <div className="panel-body">
            ...
            <div className="space-y-2">
              {predictions.map((pred: any, i: number) => (
                <div key={i} className="flex items-center justify-between p-2 bg-card rounded">
                  <span className="text-xs">{pred.type ?? pred.name ?? `Prediction ${i+1}`}</span>
                  <div className="flex items-center gap-2">
                    <div className="w-16 h-1.5 bg-muted rounded-full overflow-hidden">
                      <div className={cn("h-full rounded-full",
                        (pred.probability ?? 0) >= 60 ? 'bg-severity-high' :
                        (pred.probability ?? 0) >= 30 ? 'bg-severity-medium' : 'bg-severity-low'
                      )} style={{ width: `${pred.probability ?? 0}%` }} />
                    </div>
                    <span className="text-[10px] font-mono w-8">{pred.probability ?? 0}%</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="space-y-2">
              {recommendations.slice(0, 2).map((rec: any, i: number) => (
                <div key={i} className="p-2 bg-card rounded border-l-2 border-primary">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-xs font-medium">{rec.title ?? 'Recommendation'}</span>
                  </div>
        ...
        {!aiData && (
          <div className="text-center text-sm text-muted-foreground py-4">No AI analysis data available</div>
        )}
      </div>
    </div>
  );
